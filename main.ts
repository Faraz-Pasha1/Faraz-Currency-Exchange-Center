import inquirer from "inquirer";
import chalk from "chalk";

console.log(chalk.red("\n \t Welcome to the Faraz Pasha Currency Exchange Center! \n"));

// Define the List of Currencies and thier exchange rates
let exchange_rate: any = {
    "USD": 1,         // Base currency
    "EUR": 0.9,       // EURO
    "JYP": 113,       // Japnese
    "CAD": 1.3,       // Canadiena dollar
    "AUD": 1.65,       // australian dollar
    "PKR": 280,        // pakistani rupees
    // add more currencies as per your projects
}

// Promt the user to select a currency to convert from and to
let user_answer = await inquirer.prompt([
    {
        name : "from_currency",
        type : "list",
        message : "Select a currency to convert from",
        choices : ["USD", "EUR", "JYP", "CAD" , "AUD", "PKR"]
    },
    {
        name : "to_currency",
        type : "list",
        message : "Select a currency to convert to",
        choices : ["USD", "EUR", "JYP", "CAD" , "AUD", "PKR"]
    },
    {
        name : "amount",
        type : "input",
        message : "Enter the amount to convert"
    }
]);

// Perform the conversion of the currency by using the formula

let from_amount = exchange_rate[user_answer.from_currency];
let to_amount = exchange_rate[user_answer.to_currency];
let amount = user_answer.amount

// formula of conversion
let base_amount = amount / from_amount
let converted_amount = base_amount * to_amount

// display the converted amount
console.log(`Converted amount = ${converted_amount}`);
